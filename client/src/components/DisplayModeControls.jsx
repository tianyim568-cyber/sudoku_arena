 ...
 *
 * The server knows six display modes (see engine/DisplayModes.js), but only
 * two have a client view today: DEFAULT (the normal screen) and LIVE_RANKING
 * (the live leaderboard). PLAYER_BROADCAST is driven by the "project" button
 * in JudgeMonitoringPanel — we do not duplicate it here. The other three
 * (ROUND_RANKING, STAGE_RANKING, FINAL_RANKING) have no view on the display
 * page yet: a button that switched to one would silently fall back to the
 * default view, and the judge would believe they changed something while the
 * room saw nothing move. We do not offer a button that lies. Two modes, then.
 *
 ...
 * PLAYER_BROADCAST) and the judge clicks DEFAULT or LIVE_RANKING here, we
 * call stopBroadcast FIRST, then setDisplayMode. The reason is on the
 * server: setDisplayMode updates `display_mode` but does NOT clear
 * `broadcast_player_id` — so switching away without stopBroadcast would
 * leave a dangling id on the competition row. Calling stopBroadcast cleans
 * the projection state, then setDisplayMode sets the new mode. The screen
 * side already handles DISPLAY_MODE_CHANGED by dropping the broadcast
 * player locally, so the room sees the switch immediately.
 *
 * Visibility: the parent renders this only when isAdmin is true (ORG_ADMIN
 * or SUPER_ADMIN), which matches the server's roleMiddleware on
 * PUT /display/mode. A plain judge never sees the block and never hits a 403.
 *
...
// The two modes we expose. The server defines four more, but they have no
// client view yet — see the component doc for why we hide them.
...
export default function DisplayModeControls({ competitionId, currentMode, onModeChanged }) {
  ...
  // The two modes we expose. Order matters for the layout, not for the
  // behavior — both are independent toggles.
  const modes = [
  ...
  return (
    <section className="bg-white rounded-xl shadow p-4 sm:p-6">
      ...
      {/* When a player is being projected, surface what will happen if the
          judge clicks a mode here — the projection stops. This is the
          coherence decision made explicit for the judge. */}
      {currentMode === MODE_PLAYER_BROADCAST && (
