...
// a stale constant makes a button silently never appear, and nothing fails.
// That is exactly what happened here — this page still tested for PENDING and
// NOT_STARTED, values the server stopped writing at the competition→stage
// migration, so most of the console was inert.
//
// A competition is DRAFT → PUBLISHED → RUNNING → (PAUSED) → FINISHED.
// A stage and a round are WAITING → RUNNING/IN_PROGRESS → FINISHED.
const COMPETITION_READY_TO_START = ['DRAFT', 'PUBLISHED'];
const ROUND_NOT_STARTED = 'WAITING';
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <span className={`px-2 sm:px-3 py-1 rounded text-xs sm:text-sm font-medium ${
              COMPETITION_READY_TO_START.includes(competition.status) ? 'bg-yellow-600' :
              competition.status === 'RUNNING' ? 'bg-green-600' :
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <section className="bg-white rounded-xl shadow p-4 sm:p-6">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {COMPETITION_READY_TO_START.includes(competition.status) && (
