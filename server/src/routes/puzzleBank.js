...
function createPuzzleBankRouter(repos) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      //
      // Phase 2 deduplication: createStandalone now computes grid_hash
      // and returns the existing puzzle if a duplicate is found. The
      // `isDuplicate` flag tells us whether to skip or attach.
      const newPuzzleIds = [];
